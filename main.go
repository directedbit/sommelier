...
FOR_LOOP:
	for {
		...
		case <-didProcessCh:
			...
			MyR.Logger.Info("Flappy Trader Indexing", "BlockHeight", first.Height)
			somm_to_osmo := 0.0
			osmo_to_eth := 0.0
			if MyR.pool.IsCaughtUp() {
...
func indexDbStore(logger tmlog.Logger) {
	...
	for i := start_block; i < end_block; i++ {
		...
		pw, curPwFile, curPrefix = flappy_trade.GetParquetWriter(partquetFolder, block, logger, pw, curPwFile, curPrefix)
		if block == nil {
			panic("block is nil")
		} else {
			flappy_trade.IndexBlock(block, 0.0, 0.0, logger)
		}
		//pw.Flush(true)
	}
	//pw.WriteStop()
